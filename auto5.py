...
import trayEMenu4 # .get_profiles, .change_profile
...
class AppFinder:
    """
    Classe per la ricerca delle applicazioni installate nel sistema.
    """
    ...
    def find_apps(self, callback=None):
        ...
        if sys.platform.startswith('win'):
            # Su Windows, cerca nelle cartelle tipiche e nel registro
            self.installed_apps = self._find_windows_apps(callback)
        elif sys.platform.startswith('darwin'):
            # Su macOS, cerca nella cartella Applications
            self.installed_apps = self._find_macos_apps(callback)
        elif sys.platform.startswith('linux'):
            # Su Linux, cerca nelle directory standard
